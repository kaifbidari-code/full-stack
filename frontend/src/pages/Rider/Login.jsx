...
const RiderLogin = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const success = await login(email, password, 'rider');
    if (success) {
  ...
  return (
    <div className="flex-center">
      <div className="glass-panel animate-fade-in" style={{ width: '100%', maxWidth: '400px', position: 'relative' }}>
        ...
        <h2 style={{ textAlign: 'center', marginBottom: '2rem', marginTop: '1rem' }}>Rider Login</h2>
        ...
        <div style={{ marginTop: '1.5rem', textAlign: 'center', fontSize: '0.875rem' }}>
          Don't have an account? <Link to="/rider/register">Register here</Link>
        </div>
      </div>
    </div>
  );
